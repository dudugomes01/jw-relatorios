...
import { useForm } from "react-hook-form";
import { z } from "zod";
import { format, isToday, isFuture, compareAsc, startOfDay } from "date-fns";
import { ptBR } from "date-fns/locale";
import { Header } from "@/components/layout/header";
import { NavigationBar } from "@/components/layout/navigation-bar";
...
export default function RemindersPage() {
  ...
  const categorizedReminders = reminders.reduce(
    (acc, reminder) => {
      const reminderDate = new Date(reminder.date);
    ...
    { today: [] as Reminder[], upcoming: [] as Reminder[] }
  );
  ...
  categorizedReminders.today.sort(sortByDate);
  categorizedReminders.upcoming.sort(sortByDate);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <Card className="md:col-span-2">
            ...
            <CardContent>
              ...
              ) : categorizedReminders.today.length === 0 && categorizedReminders.upcoming.length === 0 ? (
                <p className="text-center py-8 text-muted-foreground">
                  Nenhum lembrete futuro encontrado. Crie um novo lembrete!
                </p>
              ) : (
                  ...
                  {categorizedReminders.today.length > 0 && (
                    <div>
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <h3 className="font-semibold text-primary">Hoje</h3>
                      </div>
                      <div className="space-y-4">
